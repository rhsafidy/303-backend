...
import { IProduct, IProductCategory, IProductImage } from '../../interfaces/product.interface';
...
export interface IProductRepository extends BaseRepository<IProduct> {
  ...
  findBySearch(query: string, page: number, limit: number): Promise<PaginatedProducts>;
  ...
  updateCategory(id: string, data: Partial<IProductCategory>): Promise<IProductCategory | null>;
  deleteCategory(id: string): Promise<void>;

  // ─── Images ────────────────────────────────────────────────
  addImage(productId: string, data: Partial<IProductImage>): Promise<IProductImage>;
